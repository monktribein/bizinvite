...
All five secrets are required when `NODE_ENV=production`. Outside production, a missing access token enables a
**dry-run sender**: messages are recorded with a `dryrun.*` id and logged, not sent, so the pipeline can be tested
locally. Dry-run messages are not counted as usage.
...
- **Gate.** Before any send (campaign, reminder, pass, test), the template must be `APPROVED` and every variable
  bound, otherwise `400 TEMPLATE_NOT_APPROVED` or `422 VALIDATION_ERROR`.
